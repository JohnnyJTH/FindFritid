...
<div
    class="relative flex flex-col bg-card border border-border rounded-md overflow-hidden"
>
    <div class="aspect-w-3 aspect-h-2">
        <img
            style="view-transition-name: {activity.name.replaceAll(' ', '-')};"
            class="object-cover object-center sm:w-full sm:h-full"
            src={activity.cover}
            alt={activity.name}
        />
    </div>
        ...
        <Dialog.Root>
            ...
            <Dialog.Content>
                <Dialog.Header>
                    ...
                    <Dialog.Description
                        >{activity.description}</Dialog.Description
                    >
                </Dialog.Header>
            </Dialog.Content>
        </Dialog.Root>
