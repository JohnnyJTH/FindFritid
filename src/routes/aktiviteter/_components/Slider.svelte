    ...
    <HoverCard.Root openDelay={200} closeDelay={100}>
        <HoverCard.Trigger asChild let:builder>
            <div class="grid gap-4" use:builder.action {...builder}>
                ...
                <Slider id="distance" max={100} bind:value step={5} class="[&_[role=slider]]:h-4 [&_[role=slider]]:w-4" aria-label="Afstand" />
            </div>
        </HoverCard.Trigger>
        <HoverCard.Content class="w-[260px] text-sm" side="right" align="start">Hvor langt aktiviteten må være væk.</HoverCard.Content>
    </HoverCard.Root>
</div>
