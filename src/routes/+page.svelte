    ...
    onMount(() => {
        ...
        map.on("load", () => {
            ...
            geocoder.on("result", (event) => {
                ...
                const activeListing = document.getElementById(`listing-${geoLocations.features[0].properties.id}`);
        ...
        const flyToStore = (currentFeature: (typeof geoLocations)["features"][number]) => {
            map.flyTo({
        ...
        const createPopUp = (currentFeature: (typeof geoLocations)["features"][number]) => {
